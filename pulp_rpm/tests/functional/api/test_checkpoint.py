...
def rpm_artifact_factory_class(
    pulpcore_bindings, gen_object_with_cleanup, pulp_domain_enabled, tmp_path_factory
):
    """Return an artifact created from uploading an RPM file."""

    def _rpm_artifact_factory(url=RPM_SIGNED_URL, pulp_domain=None):
        temp_file = tmp_path_factory.mktemp("content") / str(uuid.uuid4())
...
def setup(
    ...
    def create_publication(repo, checkpoint):
        ...
        return rpm_publication_factory(repository=repo.pulp_href)
